...
inline std::ostream& operator<<(std::ostream& os, const loki::Successors& suc)
{
    for (loki::Successors::const_iterator i = suc.begin(); i != suc.end(); i++) {
        if (i != suc.begin()) {
            os << endl;
        }
...
inline std::ostream& operator<<(std::ostream& os, const loki::FingerEntry& entry)
{
